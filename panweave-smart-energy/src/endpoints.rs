...
pub fn server<const A: usize>(id: ClusterId) -> Option<Result<ClusterInstance<A>, ZclStatus>> {
    Some(match id {
...
pub fn client<const A: usize>(id: ClusterId) -> Option<Result<ClusterInstance<A>, ZclStatus>> {
    Some(match id {
...
pub fn metering_server<const A: usize>() -> Result<ClusterInstance<A>, ZclStatus> {
...
pub fn add_supply_limit<const A: usize>(c: &mut ClusterInstance<A>) -> Result<(), ZclStatus> {
...
pub fn drlc_client_for<const A: usize>(device_class: u16) -> Result<ClusterInstance<A>, ZclStatus> {
...
fn drlc_client<const A: usize>() -> Result<ClusterInstance<A>, ZclStatus> {
    drlc_client_for(drlc::device_class::SIMPLE_MISC_LOADS)
}

/// A Prepayment server with the mandatory information-set attributes.
pub fn prepayment_server<const A: usize>() -> Result<ClusterInstance<A>, ZclStatus> {
...
pub fn energy_management_server<const A: usize>() -> Result<ClusterInstance<A>, ZclStatus> {
    ...
    fn every_profile_cluster_of_every_device_is_instantiable() {
        for d in DEVICES {
                ...
                .chain(common::OPTIONAL_SERVERS)
            {
                if c.0 >= 0x0700 {
                    assert!(
                        server::<36>(*c).is_some_and(|r| r.is_ok()),
                        "{} server {:?}",
                        d.name,
                        c
                    );
                ...
                .chain(common::OPTIONAL_CLIENTS)
            {
                if c.0 >= 0x0700 {
                    assert!(
                        client::<36>(*c).is_some_and(|r| r.is_ok()),
                        "{} client {:?}",
                        d.name,
                        c
                    );
        ...
        assert!(server::<36>(cluster::BASIC).is_none());
        assert!(!has_server(cluster::TIME) && has_client(cluster::PRICE));
        let m = metering_server::<36>().unwrap();
