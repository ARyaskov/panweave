...
| `green-power` / `direct` / `smart-energy` | Optional subsystems. `green-power` on `panweave-runtime` adds the Basic Proxy (`enable_green_power_proxy`) and the Basic Combo sink (`enable_green_power_sink`, `green_power_commission`); `direct` on `panweave` makes `Node` a Zigbee Direct device (`panweave::direct`, the `Zdd` trait of the Commissioning Service) and adds `Event::Direct`, and binds the Tunnel Service: `Node::open_tunnel` registers a Trusted Link interface of the NWK layer for the ZVD, `Node::on_tunnel_write` feeds decrypted NPDU Message TLVs to `Stack::on_trusted_link_npdu` (the ZVD becomes a NWK neighbour behind the link, its frames processed as NWK-secured when the TLV says so) and NPDUs the NWK layer routes to that neighbour come out as `Event::DirectTunnel` TLVs (`NwkAction::TrustedLinkData` / `StackEvent::TrustedLinkNpdu`, R23.2 NLME-TRUSTEDLINK-PRE/POSTPROCESSING); `smart-energy` on `panweave` re-exports the crate as `panweave::smart_energy` and adds `panweave::smart_energy_endpoints`, the Table 5-13 device builders on profile 0x0109, `panweave::cbke` and `panweave::smart_energy_drivers`, the cluster models bound to a `Stack`. |
